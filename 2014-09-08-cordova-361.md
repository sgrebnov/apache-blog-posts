...
Most notable changes include:
  ...
  * **Windows8** platform is now called **Windows** (you may continue to use old Windows8 alias when working with Command Line tools)
  * **Windows** platform supports building apps targeting Windows 8.1 and Windows Phone 8.1 and addition to Windows 8.0 (see documentation for new `--win` and `--phone` Command Line switches). Note that **Windows Phone 8.0** is still supported but continues to stay as a separate `wp8` platform.
